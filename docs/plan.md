...
- **Week 1: foundations.** Monorepo scaffold; `tools/export_weights.py` (MLX safetensors → compact weights binary); golden test vectors from the submodule; TS engine spike. *Gate: engine matches golden outputs and sustains 125+ steps/sec, else switch to ONNX path.*
...
| 2026-07-06 | Graves asset format | CALW v2 (`graves-v2.bin`): int8 weights with an f32 scale per output column, dequantized at load, plus per-style primed states baked at export. Download 14.6 → 3.6 MB with no visible quality loss, and styled writes start instantly instead of teacher-forcing the primer strokes for 1-2 s in the client. The f32 export (with style stroke tensors) stays as a gitignored fixture for the MLX golden tests, regenerated by `pnpm gen:weights` |
